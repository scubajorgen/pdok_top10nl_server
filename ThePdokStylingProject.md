...
**We solved this by introducing a new type of filter expression: "_IN" and "!_IN" in which you can use one or more single values. During insertion of layers using StyleConvert the filter expression is evaluated and translated to the regular 'in' resp. '!in' expression (used by MapBox GL) with all possible combinations occurring in the GPKG database (that contain the single values) as filter terms. Refer to the [StyleConvert readme](./java/StyleConvert/readme.md).**
1. Only GPS kernnetpunt is shown (red triangle), not plain white RD points (white triangle)... (not found yet, not in the data, not on TopRaster)
