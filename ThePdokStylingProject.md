...
Most of the items could be implemented using MapBox styling, however a few items had to be modified. Refer to the latest chapter for a list of deviations.
...
The styling has been done based on the **februar release 2024** of PDOK TopNL.
...
## Dynamic styling
For raster maps like TopNL Raster rendering is static: styling is done once for the most detailed zoomlevel and static images are generated. Zooming in means enalarging the image, zooming out means reducing the image. Zooming out enough means features can no longer be observed because they become to small to see.
...
* Mapbox sometimes does not show features like texts and icons if they overlap other features (decluttering), although allow_overlap is set to TRUE. This usually shows as disappearing/reappearing featues when zooming. Note that some features show when at detailed zoomlevels.
